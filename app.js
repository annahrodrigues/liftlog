...
const list=document.querySelector('#exerciseList'),template=document.querySelector('#exerciseTemplate'),cardioTemplate=document.querySelector('#cardioTemplate'),restEl=document.querySelector('#restTimer');let restSeconds=0,restActive=false;
...
function renderRoutines(){document.querySelector('#routineGrid').innerHTML=routines.map((routine,index)=>`<article class="routine-card ${routine.color}"><span>Treino ${String.fromCharCode(65+index)}</span><h2>${routine.name}</h2><p>${routine.subtitle}</p><footer><b>${routine.exercises.length} exercícios</b><button class="start-routine" data-routine="${index}">Selecionar →</button></footer></article>`).join('');document.querySelectorAll('.start-routine').forEach(button=>button.addEventListener('click',()=>startRoutine(routines[+button.dataset.routine])))}
function startRoutine(routine){if(active())return toast('Encerre o treino atual antes de selecionar outra rotina.');state.active={name:routine.name,startedAt:null,elapsed:0,timerStarted:false,exercises:routine.exercises.map(([name,sets,reps,detail])=>{const cardio=reps.includes('min')||/^Cardio\b/i.test(name);return cardio?{name,isCardio:true,cardioDone:false,cardioType:'',group:`Cardio · ${reps}`,previous:'sem histórico',rpe:'',notes:'',sets:[]}:{name,group:`${sets} séries × ${reps}${detail?` · ${detail}`:''}`,previous:'sem histórico',rpe:detail.startsWith('RPE')?detail:'',notes:'',sets:Array.from({length:sets},()=>[0,0,false])}})};save();render();goTo('treino');toast(`${routine.name} pronta. Inicie o cronômetro quando quiser.`)}
